sap.ui.define(
  ...
  function (
    ...
    var oPluginViewController = PluginViewController.extend(
      "stellium.ext.podplugins.batchCorrectionPlugin.controller.PluginView",
      {
        metadata: {
          properties: {},
        },
        ...
        onBeforeRenderingPlugin: function () {
          this.subscribe(
            "orderSelectionEvent",
            this.handleOrderSelectionEvent,
            this
          );
        ...
        handleOrderSelectionEvent: async function (
          sChannelId,
          sEventId,
          oData
        ) {
          ...
          var oBatchCorrectionInfo = await this._getBatchCorrectionData().catch(
            (oError) => {
              return [];
            }
          );
          ...
          var aLineItems = await this._getGoodsIssueSummaryForOrder(
            oData.order
          );
          //Apply the correction entries
          if (this.batchCorrItems.length > 0) {
            aLineItems.forEach((oItem) => {
              var oCorrItem = this.batchCorrItems.find(
                (val) => val.component === oItem.materialId.material
              );
              if (!oCorrItem) return;
          ...
          aLineItems.forEach((oItem) => {
            ...
            if (
              oItem.recipeComponentToleranceOver &&
              oItem.recipeComponentToleranceUnder
            ) {
            ...
            fUpperThreshold =
              oItem.targetQuantity.value * (1 + fToleranceUpper);
            fLowerThreshold =
          ...
          this._getMaterialDetails(sMaterial, sVersion).then((oMaterial) => {
            ...
            switch (oMaterial.quantityRestriction) {
              case "ANY_NUMBER":
                this.getView()
                  .getModel("viewModel")
                  .setProperty("/scaleFactor/stepSize", 0.05);
                this.getView()
                  .getModel("viewModel")
                  .setProperty("/scaleFactor/precision", 2);
                break;
              case "WHOLE_NUMBER":
              default:
                this.getView()
                  .getModel("viewModel")
                  .setProperty("/scaleFactor/stepSize", 1);
                this.getView()
                  .getModel("viewModel")
                  .setProperty("/scaleFactor/precision", 0);
            }
          });
        },
        ...
        onCalculateNewBomQty: function (oEvent) {
          ...
          var fGrQty = this.getView()
            .getModel("viewModel")
            .getProperty("/scaleFactor/value");
          var oRequestBody = {
            payload: {
          ...
          this.getView()
            .getModel("viewModel")
            .setProperty("/isCheckEnabled", false);
          ...
          this.ajaxPostRequest(
            ...
            function (oResponse) {
              ...
              aLineItems.forEach((oLineItem) => {
                ...
                var oItem = oResponse.response.calculatedBom.find(
                  (oResItem) => oResItem.component === sComponent
                );
            ...
            function (oError) {
              this.getView()
                .getModel("viewModel")
                .setProperty("/isCheckEnabled", true);
            }.bind(this)
          );
        },
        ...
        onCalculateBatchCorrectionBtnPress: function (oEvent) {
          ...
          var oBatchCorrectionItem = aLineItems.find(
            (oItem) => oItem.status === "BATCH_CORRECTION"
          );
          if (!oBatchCorrectionItem) {
            MessageBox.information(
              this.getI18nText("noItemsForBatchCorrectionMsg")
            );
            return;
          }
          ...
          this.ajaxPostRequest(
            ...
            function (oResponse) {
              ...
              this.getView()
                .getModel("viewModel")
                .setProperty("/scaleFactor/value", oResponse.response.grQty);
              this.getView()
                .getModel("viewModel")
                .setProperty("/scaleFactor/min", oResponse.response.grQty);
        ...
        onReject: function () {
          MessageBox.confirm(
            this.getI18nText("scrapSfcConfirmationMsg", [
              this.selectedOrder.sfc,
            ]),
            {
        ...
        onApprove: function () {
          ...
          if (oStepInput.getValueState() === sap.ui.core.ValueState.Error) {
            return MessageBox.error(
              this.getI18nText("fixErrorsBeforeProceedErrMsg")
            );
          }
          ...
          var oViewModel = this.getView().getModel("viewModel"),
            ...
            sMessage = this.getI18nText("confirmBatchCorrectionApproval", [
              this.selectedOrder.sfc,
              fCalcQuantity,
              fCalcQtyUom,
            ]);
        ...
        _cancelGoodsIssue: async function () {
          ...
          // try {
          var aGiLineItems = await Promise.all(aGiLineItemPromise).catch(
            (oError) => {
              Log.error("Could not load GI items information");
              // throw new Error('Could not load GI items information');
              return;
        ...
        _postDiscardOrder: function () {
          ...
          var sUrl =
            this.getPublicApiRestDataSourceUri() +
            `/order/v1/orders/discard?plant=${sPlant}&order=${sOrder}`;
          return new Promise((resolve, reject) =>
            this.ajaxPostRequest(sUrl, null, resolve, reject)
          );
        },
        ...
        _getRoutingDetailsForOrder: function (sOrderId) {
          var sUrl =
            this.getPublicApiRestDataSourceUri() + "routing/v1/routings";
          var oParams = {
            plant: this.getPodController().getUserPlant(),
            routing: sOrderId,
        ...
        _getGoodsIssueItemsForMaterial: function (oItem) {
          var sUrl =
            this.getPublicApiRestDataSourceUri() +
            "inventory/v1/inventory/goodsIssues";
          var oParams = {
            plant: this.getPodController().getUserPlant(),
        ...
        _getGoodsIssueSummaryForOrder: function (sOrderId) {
          ...
          var sUrl =
            this.getPodController().getAssemblyDataSourceUri() +
            "order/goodsIssue/summary";
          var aPromises = aRoutingSteps.map((oStep) => {
            var sOperationActivity =
              oStep.routingOperation.operationActivity.operationActivity;
        ...
        _postTecoToS4: function () {
          var sUrl =
            this.getPublicApiRestDataSourceUri() +
            "/pe/api/v1/process/processDefinitions/start?key=REG_39e69f0f-b61d-4d46-9274-5123baa99842";
          var oPayload = {
            content: {
        ...
        _postSfcScrap: async function () {
          ...
          var oRequestBody = {
            ...
            quantity:
              oGRSummary.quantityInBaseUnit.value ||
              oGRSummary.targetQuantityInProductionUnit.value,
            operation: oBatchCorrectionItem.operationActivity,
          };

          return new Promise((resolve, reject) =>
            this.ajaxPostRequest(sUrl, oRequestBody, resolve, reject)
          ).then((oResponse) => {
            MessageToast.show(
              this.getI18nText("sfcScrapped", [this.selectedOrder.sfc])
            );
            this.navigateToPage("MainPage");
        ...
        _releaseSfcHold: function () {
          var sUrl =
            this.getPublicApiRestDataSourceUri() + "sfc/v1/sfcs/release";
          var oRequestBody = {
            plant: this.getPodController().getUserPlant(),
            sfcs: [this.selectedOrder.sfc],
        ...
        _getGoodsReceiptSummary: function () {
          var sUrl =
            this.getPublicApiRestDataSourceUri() +
            "inventory/v1/inventory/goodsReceipts/summarize";
          var oParams = {
            plant: this.getPodController().getUserPlant(),
            sfc: this.selectedOrder.sfc,
            order: this.selectedOrder.order,
        ...
        _sendBatchCorrectionToS4: function () {
          var sUrl =
            this.getPublicApiRestDataSourceUri() +
            "/pe/api/v1/process/processDefinitions/start?key=REG_602cf830-1ee2-4756-bd82-e306ef25940a";
